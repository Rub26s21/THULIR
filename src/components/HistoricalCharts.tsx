// ============================================================
// THULIR - Ultra-Premium Chromatic Historical Trends Panel
// ============================================================
...
const METRIC_TABS: { key: MetricTab; label: string; color: string }[] = [
  { key: 'ALL', label: 'All Sensors (Grid)', color: '#06b6d4' },
  { key: 'TILT', label: 'Tilt X / Y', color: '#06b6d4' },
  { key: 'PRESSURE', label: 'Pressure', color: '#3b82f6' },
  { key: 'GAS', label: 'Gas (MQ-2)', color: '#f59e0b' },
  { key: 'ENV', label: 'Temp & Humidity', color: '#ef4444' },
  { key: 'DISTANCE', label: 'Distance', color: '#f97316' },
    ...
    <div className="skeuo-card" role="region" aria-label="Live Sensor Trends">
      ...
      <div className="charts-controls-bar">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: '0.88rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
      ...
      <div className="charts-metric-tabs" style={{ marginBottom: 16 }}>
        {METRIC_TABS.map(tab => (
          <button
            ...
            style={{
              borderColor: activeTab === tab.key ? `${tab.color}80` : undefined,
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '40px 0' }}>
          <div style={{ fontSize: '0.82rem', color: 'var(--accent-cyan)', fontFamily: 'JetBrains Mono, monospace' }}>
      ...
      {error && (
        <div style={{ padding: '12px', background: 'var(--status-critical-bg)', border: '1px solid var(--status-critical-border)', borderRadius: 8, color: 'var(--status-critical)', fontSize: '0.8rem', marginBottom: 12 }}>
          Chart query notice: {error}
        </div>
      )}

      {!loading && !error && formattedHistory.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-dim)', fontSize: '0.85rem' }}>
          No telemetry records found for the selected {timeRange} time range.
        </div>
      )}
        ...
        <div>
                ...
                return (
                  <div key={meta.key} className="skeuo-well" style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: '0.75rem', fontWeight: 600 }}>
                      ...
                      <span style={{ color: color, fontFamily: 'JetBrains Mono, monospace', fontWeight: 700 }}>{meta.unit}</span>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height={155}>
                      <AreaChart data={formattedHistory} margin={{ top: 5, right: 10, left: -15, bottom: 0 }}>
                        ...
                        <Tooltip
                          contentStyle={{ background: 'var(--chart-tooltip-bg)', border: '1px solid var(--chart-tooltip-border)', borderRadius: 8, fontSize: 11, color: 'var(--text-primary)', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}
                          formatter={(val: any) => [`${val ?? 'N/A'} ${meta.unit}`, meta.name]}
                        />
                        {threshold && (
                          <ReferenceLine y={threshold.critical} stroke="var(--status-critical)" strokeDasharray="3 3" opacity={0.6} />
                        )}
          ...
          {activeTab === 'TILT' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>MPU6050 Structural Inclination (Tilt X vs Tilt Y)</div>
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={formattedHistory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="tilt_x" stroke="#06b6d4" name="Tilt X (°)" strokeWidth={2.4} dot={false} />
          ...
          {activeTab === 'PRESSURE' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>BMP280 Barometric Atmospheric Pressure</div>
              <ResponsiveContainer width="100%" height={280}>
          ...
          {activeTab === 'GAS' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>MQ-2 Analog Gas Concentration (Raw ADC 0–1023)</div>
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={formattedHistory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  <defs>
          ...
          {activeTab === 'ENV' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>DHT22 Environmental Metrics (Temperature &amp; Humidity)</div>
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={formattedHistory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" />
                  <XAxis dataKey="timeFormatted" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickLine={false} />
          ...
          {activeTab === 'DISTANCE' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>HC-SR04 Displacement Clearance Distance</div>
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={formattedHistory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--chart-grid)" />
                  <XAxis dataKey="timeFormatted" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickLine={false} />
          ...
          {activeTab === 'VIBRATION' && (
            <div className="skeuo-well" style={{ padding: '16px' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: 10 }}>ADXL345 50-Sample Vibration RMS Magnitude</div>
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={formattedHistory} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                  <defs>
